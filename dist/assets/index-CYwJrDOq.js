(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))i(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const s of o.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&i(s)}).observe(document,{childList:!0,subtree:!0});function n(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(a){if(a.ep)return;a.ep=!0;const o=n(a);fetch(a.href,o)}})();const d=[{src:new URL("/assets/advogado-CbLRrGfx.png",import.meta.url).href,caption:"Landing Page para Advogado - Atuando internacionalmente"}];let r=0;function h(){document.getElementById("lightbox").addEventListener("click",function(e){e.target===this&&c()}),document.addEventListener("keydown",function(e){document.getElementById("lightbox").classList.contains("active")&&(e.key==="Escape"&&c(),e.key==="ArrowLeft"&&l(-1),e.key==="ArrowRight"&&l(1))})}function b(e){r=e,f(),document.getElementById("lightbox").classList.add("active"),document.body.style.overflow="hidden"}function c(){document.getElementById("lightbox").classList.remove("active"),document.body.style.overflow=""}function l(e){r=(r+e+d.length)%d.length,f()}function f(){const e=d[r];document.getElementById("lb-img").src=e.src,document.getElementById("lb-img").alt=e.caption,document.getElementById("lb-caption").textContent=e.caption}function A(){const e=document.getElementById("dashboard-carousel"),t=document.getElementById("carousel-prev"),n=document.getElementById("carousel-next");t.addEventListener("click",()=>p(e,-1)),n.addEventListener("click",()=>p(e,1))}function p(e,t){const n=e.querySelector(".dashboard-thumb"),i=parseInt(getComputedStyle(e).gap)||20,a=(n?n.offsetWidth:320)+i;e.scrollBy({left:t*a,behavior:"smooth"})}const y={endpoint:"/api/chat"};async function S(e){const t=await fetch(y.endpoint,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:e})});if(!t.ok)throw new Error(`AI API error: ${t.status}`);return(await t.json()).choices[0].message.content}const O=`
...
"Sou um assistente dedicado exclusivamente a responder perguntas sobre Osman Bastos e Silva Cruz."
`;function E(){document.body.appendChild(I());const e=document.getElementById("ai-chat-toggle"),t=document.getElementById("ai-chat-panel"),n=document.getElementById("ai-chat-close"),i=document.getElementById("ai-chat-input"),a=document.getElementById("ai-chat-send"),o=document.getElementById("ai-chat-messages"),s=[{role:"system",content:O}];m(o,"Olá! Sou o assistente virtual do Osman. Posso responder perguntas sobre sua experiência, stack técnica, projetos ou disponibilidade. Como posso ajudar?"),e.addEventListener("click",()=>t.classList.toggle("open")),n.addEventListener("click",()=>t.classList.remove("open")),a.addEventListener("click",()=>g(i,a,o,s)),i.addEventListener("keydown",v=>{v.key==="Enter"&&g(i,a,o,s)})}async function g(e,t,n,i){const a=e.value.trim();if(!a||t.disabled)return;P(n,a),e.value="",t.disabled=!0,i.push({role:"user",content:a});const o=C(n);try{const s=await S(i);i.push({role:"assistant",content:s}),o.remove(),m(n,s)}catch(s){o.remove(),m(n,"Desculpe, ocorreu um erro. Tente novamente mais tarde."),console.error(s)}finally{t.disabled=!1,e.focus()}}function I(){const e=document.createElement("div");return e.id="ai-chat-widget",e.innerHTML=`
  ...
  `,e}function P(e,t){u(e,t,"user")}function m(e,t){u(e,t,"bot")}function C(e){return u(e,"Digitando...","bot typing")}function u(e,t,n){const i=document.createElement("div");return i.className=`ai-msg ${n}`,i.textContent=t,e.appendChild(i),e.scrollTop=e.scrollHeight,i}document.getElementById("year").textContent=new Date().getFullYear();h();A();E();window.openLb=b;window.closeLb=c;window.lbNav=l;
